...
{
    /// <summary>One player's preview payload for one slot, exactly as published (#280).</summary>
    public readonly record struct PreviewEntry(PlayerID SourcePlayerID, string Slot,
        string PreviewTypeName, string PreviewJson);

    /// <summary>
    /// Receive side of live decision-preview sharing (#280): the latest preview state of every
