...
namespace FDG.Tests
{
    ...
    public class AiDefineMovementResolverTests
    {
        ...
        public async Task Resolve_StraightLineWouldClipImpassible_ResultIsEngineValid()
        {
            ...
            var mover = new ModelData(0.75f, new List<Weapon>(), new List<SpecialRule>(), new Position(0f, 5f), store);
            var moverBinding = store.GetDataBinding<ModelData>(store.Create(mover));
            var moverUnit = new UnitData(selfPlayer, "Movers", 4, 4, new List<SpecialRule>(),
            ...
            var enemy = new ModelData(0.75f, new List<Weapon>(), new List<SpecialRule>(), enemyPos, store);
        ...
        public async Task Resolve_ClearLane_AdvancesTowardEnemy()
        {
            ...
            {
                var m = new ModelData(0.75f, new List<Weapon>(), new List<SpecialRule>(), new Position(0f, z), store);
                moverBindings.Add(store.GetDataBinding<ModelData>(store.Create(m)));
            }
            var moverUnit = new UnitData(selfPlayer, "Movers", 4, 4, new List<SpecialRule>(), moverBindings);
            var moverUnitBinding = store.GetDataBinding<UnitData>(store.Create(moverUnit));
