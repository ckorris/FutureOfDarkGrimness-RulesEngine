...
namespace FDG.Tests
{
    ...
    public class AiPlaceObjectsResolverTests
    {
        ...
        public async Task DoesNotPlaceModelsOnImpassibleTerrain()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var m = new ModelData(0.75f, new List<Weapon>(), new List<SpecialRule>(), new Position(0f, 0f), store);
                placing.Add(store.GetDataBinding<ModelData>(store.Create(m)));
            }
            var unit = new UnitData(selfPlayer, "Warriors", 4, 4, new List<SpecialRule>(), placing);
            store.Create(unit);
