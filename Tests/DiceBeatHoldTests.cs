...
{
    /// <summary>
    /// #322 — a dice roll paces its FULL envelope. The gap between two rolls is the rhythm of the
    /// exchange, not dead time, so the engine still stops for each one; the "let it linger" half of #322
    ...
    /// then #322 itself after playing it). If the default flips again, combat silently reads as rushed —
    ...
    public class DiceBeatHoldTests
    {
        ...
        public async Task AMultiThresholdVolley_PacesEachRollIdentically()
        {
            ...
            // envelope, so neither flicks past — and #322's stack means the first panel is still on
